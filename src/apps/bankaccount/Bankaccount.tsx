...
export const BankAccount = ({ codes }) => {
...
const AccountWrapper = ({ country, children }) => {
...
function getRandomBic({ country }): string {
  const bics: string[] = getBicList(country)

  const bic: string = bics.length
    ? bics[[GenerateRandomNumber(0, bics.length - 1)]]
    : GenerateRandomNumber(0, 999).toString().padStart(3, "0")

  return bic
}

function getBicList(country): string[] {
  ...
  const bics: string[] = bicValues.reduce((codes: any, currentValue) => {
    if (currentValue["countryCode"] === country.toUpperCase()) {
...
function getDigits(count: number): number {
  const digits = GenerateRandomNumber(
    Math.pow(10, count - 1),
    Math.pow(10, count) - 1
  )
